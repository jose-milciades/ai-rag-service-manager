...
**Estado: implementado parcialmente y verificado (2026-08-11), ver `pendientes.md` P-28.** El frente 1 (tool de búsqueda) y una parte del frente 2 (migración de `company_document_query.py`) ya están en código en `dev`, verificados con `ai-rag-service-manager` real. Quedan pendientes dos piezas de contenido/configuración en base de datos que no se pueden completar desde este repo (sección 3.4), y el resto de los consumidores de storage (sección 4).
...
- **Sigue bloqueado por 3.4**: hasta que exista la fila de `CatPrompt` (con `id_project = NULL`, ver el hallazgo de arriba), este endpoint responde `500` en el paso de `invoke_model` — el resto del flujo (validación de proyecto, resolución de colección, búsqueda real) ya funciona.
...
- [ ] Resolver 3.4 (fila de `CatPrompt` con `id_project = NULL` + fila de `cat_tools`/`tools_implemented`) — a cargo del usuario/Analista, no es código. Una vez creada, volver a correr `/rag-document-search/simulate` para confirmar que responde `200` con una respuesta real del LLM.
- [ ] Confirmar 3.2 con el equipo (mismo espacio de `id_project` entre `edi-ai-operator` y Java) — dato adicional a favor: el proyecto real usado para probar (`id=70`, "Event Express") existe en la base de `edi-ai-operator`; falta cruzarlo contra la base de Java para confirmar que el mismo id refiere al mismo proyecto real.
...
- [ ] Migrar `thought_persistence_service.py` (upload) y `chat_history_service.py`/`context_memory_service.py` (download) del blob de `ContextMemory`/thoughts a `rag_service_client` (`/api/v1/storage/upload` / `/api/v1/storage/get`).
- [ ] Revisar los consumidores restantes de `StorageService` listados en 1.3 (`comun_service.py`, `report_service.py`, `ia_functions.py`, `ia_functions_cache.py`, `common_duck_db.py`) uno por uno — no se migraron en esta pasada.
- [ ] Una vez migrados todos los consumidores, eliminar `src/service/util/storage_service.py` y la dependencia `google-cloud-storage` del proyecto.
- [ ] Probar `rag_document_search` integrada al agente completo (moderator/planner/selector de tools), una vez resuelto 3.4.
