...
| P-40 | `POST /dataDocType/saveDocuments` (Java, el flujo real del frontend) nunca disparaba vectorización para recursos `data_base_embedding`; + bug real `listParameters: null` en `VectorStoreMapper` (422 en `ai-rag-service-manager`) | Alta | Resuelto |
...
- **Acción sugerida:** ninguna — resuelto. `saveDocumentsResources` queda documentado como pendiente futuro si se necesita el mismo fix ahí.
