...
- **Estado:** Resuelto — la tool `rag_document_search` está verificada end-to-end de forma aislada (real, con LLM real, ver ronda 3 abajo) y el corte de storage está completo y verificado (ver ronda 4 abajo). Falta exclusivamente la fila de `cat_tools`/`tools_implemented` (a cargo del usuario) para integrarla al agente completo — no es un bloqueante de código.
- **Detectado:** 2026-08-11.
...
- **Pendiente para continuar (checklist actualizado, ver también `integracion-operator-rag.md`):**
  ...
  - [ ] Agregar la fila de worker/tool (`cat_tools`/`tools_implemented`) para que el agente pueda seleccionar la tool — a cargo del usuario, sigue pendiente (no bloquea `/simulate`, sí bloquea la integración al agente completo). Único ítem restante de este pendiente.
...
- **Hallazgo colateral, reportado sin corregir (fuera de alcance):** tanto `.env` (con `DATABASE_PASSWORD` real) como `edward-creds.json` (credencial real de GCS) están **versionados en git** en este repo (confirmado con `git ls-files`, no están en `.gitignore`) — mismo tipo de hallazgo que los secretos en texto plano de `edi-ai-proyectos-backend` (ver `integracion-java-storage.md`). No se purgó del historial ni se rotaron credenciales; se dejó de referenciar `edward-creds.json` desde el código (ya no hace falta, no hay más cliente GCS), pero el archivo sigue en el repo y en el historial de git.
- **Verificación real:** import real de los módulos tocados y de `src.main:app` completo (con lifespan/routers/CORS) — arranca sin error, 19 rutas registradas. Grep final sin residuos de `google.cloud`/`StorageService`/config de storage. `uv lock`+`uv sync` resolvieron 36 paquetes (antes 52) sin errores. **No se pudo correr `black --check`**: el grupo `dev` de `pyproject.toml` no está declarado como `[dependency-groups]`/`[project.optional-dependencies]` válido para `uv` — preexistente, no introducido por este cambio, fuera de alcance corregir aquí; se verificó en su lugar con `py_compile`. **No se probó en runtime real** ninguna llamada HTTP a `ai-rag-service-manager` — no hay todavía ningún caller de negocio que ejercite `rag_service_client.py` (ver hallazgo principal arriba), así que no hay flujo end-to-end que probar hasta que este servicio implemente una feature real de storage.
