...
| P-40 | `POST /dataDocType/saveDocuments` (Java, flujo real del frontend) nunca disparaba vectorización; + `listParameters: null` (422 real) en `VectorStoreMapper`; + `SaveEmbeddingFileResponse.successful` no matcheaba `"success"` real (isVectorized quedaba siempre null) | Alta | Resuelto |
...
### P-40 — `POST /dataDocType/saveDocuments` (Java) nunca disparaba vectorización, aunque el recurso fuera `data_base_embedding` — y bug real de `listParameters: null` descubierto al corregirlo

- **Estado:** Resuelto (dos hallazgos relacionados, mismo pendiente).
- **Detectado:** 2026-08-19/21, a partir de dos capturas reales del frontend (`curl` reales, no simulados) mostrando el flujo real que usa producción/dev: `POST /storage/upload` (guarda el archivo) seguido de `POST /dataDocType/saveDocuments` (con `idResource:25, idArea:76, idProject:93` — el mismo recurso `data_base_embedding` usado en P-34/P-36/P-39) — el documento quedaba guardado pero nunca se vectorizaba.
- **Resuelto el:** 2026-08-21.
- **Ubicación:** `edi-ai-proyectos-backend`: `dynamicDocument/DataByDocTypeManager.java` (`saveDocuments`), `business/general/mapper/VectorStoreMapper.java`.

**Hallazgo 1 — `saveDocuments` nunca llamaba a `VectorStoreService`:**
- Se rastreó la cadena completa de llamadas real: `DataByDocTypeManager.saveDocuments` → `dataByDocTypeService.saveAllDocuments` (INSERT plano en Postgres) → `TicketManager.createUpdateKnowledgeBaseDocument` → `KnowledgeBaseCommonServiceImpl.saveKnowledgeBaseDocuments` (otro INSERT plano, en una tabla `KnowledgeBase`/`KnowledgeBaseDocument` — un concepto de vinculación del módulo de tickets, **no** el vector store) — **cero referencias a `vectorStoreService`/Milvus en toda la cadena**, confirmado por grep exhaustivo. `resource` (con su `resourcesType`) ya estaba disponible en el método (se usa para setear `Document.resource`), pero nunca se leía para decidir vectorizar.
- Es el mismo defecto conceptual que P-36 (recurso `data_base_embedding` no dispara vectorización), pero en un **módulo distinto** (`dynamicDocument`/`tickets`, no `aiResourcesManagement`) — y es, a diferencia de lo que se había probado en P-34/P-36/P-39, **el flujo que realmente usa el frontend en producción/dev** (confirmado con las capturas reales de red del usuario).
- El endpoint hermano `saveDocumentsResources` tiene el mismo gap — **no corregido a propósito** (fuera de lo pedido), dejado como nota explícita en el código.
- **Implementación:** nuevo método `DataByDocTypeManager.triggerVectorizationIfDataBaseEmbedding(Resources, List<Document>)` — mismo criterio exacto que `ResourcesServiceImpl.triggerVectorizationIfDataBaseEmbedding` de P-36 (chequea `resourcesType.code == data_base_embedding`, resuelve `indexVecstore = "project-" + resource.area.project.id`, llama a `vectorStoreService.saveDocumentOnVecstoreAsync` por cada documento), adaptado a lista porque `saveDocuments` puede recibir varios documentos a la vez. Se inyectó `VectorStoreService` como dependencia nueva (Lombok `@AllArgsConstructor`, sin constructor manual que tocar).

**Hallazgo 2 — bug real preexistente, descubierto al verificar el hallazgo 1 en un ambiente donde Keycloak sí respondía:** `VectorStoreMapper.buildSaveFileVecstoreRequestKnowledgeBase` y `buildSaveVecstoreReference` nunca seteaban `SaveFileVecstoreRequest.listParameters` — quedaba en `null` (default de Lombok `@Builder`), Jackson lo serializaba como `"listParameters": null`, y el schema real de `ai-rag-service-manager` (`SaveDocumentVecstoreRequest.list_parameters: list[dict[str, Any]]`, sin `Optional`) lo rechazaba con `422 Unprocessable Entity: "Input should be a valid list"`. **No es un bug introducido por el hallazgo 1** — afecta a *todo* caller de `saveDocumentOnVecstoreAsync`/`saveReferenceUrlOnVecstoreAsync` (incluyendo `KnowledgeBaseManager`, `UserServiceImpl`, `TicketManager`, `EvaluationMaturityManager`, y la propia `ResourcesServiceImpl` de P-36) — simplemente nunca se había visto porque ninguna llamada anterior había pasado de la barrera de Keycloak (certificado vencido de `ediaimx`, ver P-34) hasta llegar a la validación real de `ai-rag-service-manager`. Se detectó con un log real del usuario contra un ambiente donde Keycloak sí respondía (`ediaidev`, el mismo host válido que usa `edi-ai-operator`).
- **Implementación:** agregado `.listParameters(List.of())` a ambos builders — mismo criterio que ya usaba `StorageManager.sendAsyncRequestVectored` (que sí seteaba una lista real después de construir el request con `buildSaveFileVecstoreRequest`, el tercer builder de esta clase, no afectado por este bug).
- **Verificación real:**
  1. `./gradlew compileJava` → `BUILD SUCCESSFUL` (los errores que mostraba el analizador del IDE eran falsos positivos por un problema de carga del annotation processor de Lombok en ese entorno, no del compilador real).
  2. **Reproducción exacta contra `ai-rag-service-manager` real:** el mismo payload que mandaba el código viejo (`listParameters: null`) reproduce el 422 real, byte a byte igual al del log del usuario; el mismo payload con `listParameters: []` (lo que manda el código corregido) responde `200`.
  3. Flujo completo replicado localmente (`POST /storage/upload` real + `POST /dataDocType/saveDocuments` real, mismos `idResource`/`idArea`/`idProject` que las capturas del usuario): confirmado en el log real de `ai-rag-service-manager` un `GET /storage/getFileByte` que **antes del hallazgo 1 nunca ocurría** desde este endpoint — prueba de que el trigger de vectorización sí se dispara.
  4. **Confirmación en el ambiente real del usuario (2026-08-21), no solo local:** con el hallazgo 2 ya corregido y desplegado, el usuario corrió el flujo real completo (`storage/upload` → `dataDocType/saveDocuments`, documento 5996, resource 25, proyecto 93) — logs reales confirman `Disparando vectorizacion para documento 5996 (resource 25, data_base_embedding) en project-93` (la línea del hallazgo 1), Keycloak respondiendo token real (`ediaidev`, válido), `POST .../embedding/save_document_vecstore` → `200` (ya no 422 — confirma el hallazgo 2), con un embedding real de OpenAI de por medio.

**Hallazgo 3 — bug real preexistente, encontrado al verificar el hallazgo 2 en el ambiente real del usuario:** pese al `200` real de `save_document_vecstore`, `document_.is_vectorized` quedó en `NULL` en Postgres (verificado con consulta real, `id=5996`) en vez de `true`. Causa: `SaveEmbeddingFileResponse` (el DTO de respuesta del lado Java) tenía el campo `Boolean successful`, pero `ai-rag-service-manager` responde `{"success": true, ...}` (`SaveDocumentVecstoreResponse.success: bool`) — nombres distintos, Jackson nunca los matchea y deja el campo en `null`. `VectorStoreServiceImpl`/`VectorStoreManager` pasan ese `null` directo a `documentCommonService.updateDocumentIsVectorized(isVectorized, uniqueCode)`, que lo persiste tal cual (sin guard contra `null`). **Mismo patrón que el hallazgo 2**: no es un bug de este pendiente, afecta a *todo* caller de `saveEmbeddingFile` en el sistema (`StorageManager`, `VectorStoreServiceImpl` x2, `VectorStoreManager`) — nunca se había detectado porque, hasta este pendiente, ninguna llamada real había llegado tan lejos (Keycloak vencido, luego el 422 del hallazgo 2).
- **Implementación:** renombrado el campo del record `SaveEmbeddingFileResponse.successful` → `success` (coincide ahora con la clave JSON real), actualizados los 4 call sites (`VectorStoreServiceImpl.java` x2, `VectorStoreManager.java`, `StorageManager.java`) de `.successful()` a `.success()`.
- **Verificación real:** `./gradlew compileJava` → `BUILD SUCCESSFUL`. **Prueba real de deserialización Jackson** (mismas versiones exactas de `jackson-databind`/`jackson-core`/`jackson-annotations` que usa el proyecto, tomadas del cache real de Gradle) con el JSON real devuelto por `ai-rag-service-manager` (`{"success":true,"message":"Document indexed successfully","uniqueCode":"mt2ekrgm9w8luvyj","chunksCreated":26,"indexName":"project-93"}`, el mismo `uniqueCode` real del documento 5996 del usuario): con el campo viejo (`successful`) deserializa a `null` (reproduce el bug exacto observado); con el campo nuevo (`success`) deserializa a `true`.
- **Acción sugerida:** ninguna — resuelto. `saveDocumentsResources` (hallazgo 1) queda documentado como pendiente futuro si se necesita el mismo fix ahí.
