...
app:
  rag-service:
    ...
    rag-query: ${app.rag-service.base-url}/embedding/rag_query
...
- [x] Java: implementar cliente HTTP a `/api/v1/storage/*` (`RagServiceStorageClient`, nueva clase, implementa la misma interfaz `StorageService`). **No activado por defecto** — `StorageServiceImpl` (GCS directo) se marcó `@Primary` a propósito, así que el comportamiento de storage no cambió. Ver nota de riesgo abajo.
...
- [ ] Decidir y ejecutar el corte de storage: cambiar el `@Qualifier` en `StorageManager` para inyectar `RagServiceStorageClient` en vez de `StorageServiceImpl`, **después** de probar en un ambiente real (dev) que sube/baja archivos correctamente contra `ai-rag-service-manager`. No se activó en esta pasada por no poder levantar ni probar este microservicio Java desde el entorno donde se hizo el cambio.
- [ ] Probar de punta a punta en un ambiente real: upload simple, upload por chunks, borrado de índice, búsqueda semántica, y el camino de error (documento no vectorizable, servicio caído).
- [ ] Confirmar si se retira el bean de GCS (`StorageConfig.java`) y las credenciales asociadas en Java, una vez que `RagServiceStorageClient` esté activo y confirmado en producción.

**Nota de riesgo (por qué el corte de storage no se activó automáticamente):** a diferencia de la vectorización (un cambio de URL, ya validado de punta a punta desde el lado `ai-rag-service-manager`), reemplazar el storage implica credenciales y buckets de GCS reales, sin forma de probar este microservicio Java end-to-end desde este entorno. Se prefirió dejar `RagServiceStorageClient` implementado y listo, pero no activo, para que el equipo lo corte deliberadamente después de probarlo en dev — no como un flag de configuración que alguien active sin querer.
